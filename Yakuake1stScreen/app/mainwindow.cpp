...
{
    QDBusConnection::sessionBus().registerObject(QStringLiteral("/Yakuake1stScreen/window"), this, QDBusConnection::ExportScriptableSlots);
...
void MainWindow::setupActions()
{
    ...
    action->setText(xi18nc("@action", "Open/Retract Yakuake1stScreen"));
    action->setIcon(QIcon::fromTheme(QStringLiteral("Yakuake1stScreen")));
...
void MainWindow::applySettings()
{
    ...
    if (!Settings::showSystrayIcon() && m_notifierItem) {
        ...
        m_notifierItem->setIconByName(QStringLiteral("Yakuake1stScreen-symbolic"));
...
void MainWindow::applySkin()
{
    ...
    if (!gotSkin) {
        KMessageBox::error(parentWidget(),
                           xi18nc("@info",
                                  "<application>Yakuake1stScreen</application> was unable to load a skin. It is likely that it was installed incorrectly.<nl/><nl/>"
...
void MainWindow::_toggleWindowState()
{
    ...
    if (visible && !isActiveWindow() && Settings::keepOpen()) {
        ...
        if (Settings::toggleToFocus()) {
            ...
            // Yakuake1stScreen. Just forcing focus would cause the window manager to
...
int MainWindow::getScreen()
{
    if (Settings::screen() <= 0 || Settings::screen() > m_outputOrderWatcher->outputOrder().length()) {
        ...
        // Right after unplugging an external monitor and the Yakuake1stScreen window was on
    ...
    if (m_outputOrderWatcher->outputOrder().count() > 1) {
        ...
        while (i.hasNext()) {
            ...
            if (KX11Extras::hasWId(windowId)) {
                ...
                // desktop with the Yakuake1stScreen window...
...
void MainWindow::updateTrayTooltip()
{
    ...
    if (!shortcuts.isEmpty()) {
        ...
        m_notifierItem->setToolTip(QStringLiteral("Yakuake1stScreen"), QStringLiteral("Yakuake1stScreen"), xi18nc("@info", "Press <shortcut>%1</shortcut> to open", shortcut));
    }
}
