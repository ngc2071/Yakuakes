...
{
    QDBusConnection::sessionBus().registerObject(QStringLiteral("/Yakuake2ndScreen/window"), this, QDBusConnection::ExportScriptableSlots);
...
void MainWindow::setupActions()
{
    ...
    action->setText(xi18nc("@action", "Open/Retract Yakuake2ndScreen"));
    action->setIcon(QIcon::fromTheme(QStringLiteral("Yakuake2ndScreen")));
...
void MainWindow::applySettings()
{
    ...
    if (!Settings::showSystrayIcon() && m_notifierItem) {
        ...
        m_notifierItem->setIconByName(QStringLiteral("Yakuake2ndScreen-symbolic"));
...
void MainWindow::applySkin()
{
    ...
    if (!gotSkin) {
        KMessageBox::error(parentWidget(),
                           xi18nc("@info",
                                  "<application>Yakuake2ndScreen</application> was unable to load a skin. It is likely that it was installed incorrectly.<nl/><nl/>"
...
void MainWindow::_toggleWindowState()
{
    ...
    if (visible && !isActiveWindow() && Settings::keepOpen()) {
        ...
        if (Settings::toggleToFocus()) {
            ...
            // Yakuake2ndScreen. Just forcing focus would cause the window manager to
...
int MainWindow::getScreen()
{
    if (Settings::screen() <= 0 || Settings::screen() > m_outputOrderWatcher->outputOrder().length()) {
        ...
        // Right after unplugging an external monitor and the Yakuake2ndScreen window was on
    ...
    if (m_outputOrderWatcher->outputOrder().count() > 1) {
        ...
        while (i.hasNext()) {
            ...
            if (KX11Extras::hasWId(windowId)) {
                ...
                // desktop with the Yakuake2ndScreen window...
...
void MainWindow::updateTrayTooltip()
{
    ...
    if (!shortcuts.isEmpty()) {
        ...
        m_notifierItem->setToolTip(QStringLiteral("Yakuake2ndScreen"), QStringLiteral("Yakuake2ndScreen"), xi18nc("@info", "Press <shortcut>%1</shortcut> to open", shortcut));
    }
}
